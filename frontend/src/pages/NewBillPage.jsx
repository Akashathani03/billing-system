...
function InvoiceGeneratedScreen({ invoice, onNewBill }) {
  return (
    <div className="flex min-h-[70vh] flex-col items-center justify-center px-6 text-center">
      ...
      <h1 className="mt-4 text-xl font-semibold text-neutral-900">Bill Generated</h1>
      ...
      <p className="text-2xl font-bold text-neutral-900">₹{invoice.total.toFixed(2)}</p>
 ...
 * via `key` whenever the route's draft id changes. Because of that, every
 * piece of state can be initialized directly from `initialInvoice` with a
 * lazy useState initializer — no effect is needed to sync fetched data into
 * local state after the fact.
 */
function NewBillForm({ initialInvoice }) {
  const navigate = useNavigate();

  const [currentId, setCurrentId] = useState(() => initialInvoice?._id || null);
  const [customer, setCustomer] = useState(() => (initialInvoice ? customerFromInvoice(initialInvoice) : null));
  const [items, setItems] = useState(() => (initialInvoice ? itemsFromInvoice(initialInvoice) : []));
  const [paymentMethod, setPaymentMethod] = useState(() => initialInvoice?.paymentMethod || 'cash');
  const [paymentStatus, setPaymentStatus] = useState(() => initialInvoice?.paymentStatus || 'paid');
  ...
  function handleSelectProduct(product) {
    setItems((prev) => {
      const existingItem = prev.find((i) => i.productId === product._id);
      if (existingItem) {
  ...
  async function ensureSaved() {
    ...
    if (currentId) {
      const { invoice } = await updateInvoice.mutateAsync({ id: currentId, data: payload });
      return invoice;
    }
  ...
  async function handleGenerateInvoice() {
    ...
    try {
      ...
      const { invoice } = await finalizeInvoiceMutation.mutateAsync(saved._id);
  ...
  if (finalizedInvoice) {
    return <InvoiceGeneratedScreen invoice={finalizedInvoice} onNewBill={resetForm} />;
  }
    ...
    <div className="px-4 pt-6 pb-40">
      ...
      <section className="mt-6">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setProductPickerOpen(true)} className="text-sm font-semibold text-blue-700">
            + Add Product
          </button>
        </div>
        ...
        <div className="mt-2 space-y-2">
          ...
          {items.map((item) => (
            <ItemCard
              key={item.productId}
              item={item}
              onQuantityChange={(q) => handleQuantityChange(item.productId, q)}
      ...
      {items.length > 0 && (
        <section className="mt-6 rounded-lg border border-neutral-200 bg-white p-4">
          ...
          <div className="mt-1 flex justify-between text-sm text-neutral-600">
            <span>GST {(preview.taxRate * 100).toFixed(0)}%</span>
      ...
      <section className="mt-6">
        <h2 className="text-sm font-semibold text-neutral-700">Payment Method</h2>
        <div className="mt-2">
      ...
      <section className="mt-4">
        <h2 className="text-sm font-semibold text-neutral-700">Payment Status</h2>
        <div className="mt-2">
          <PaymentStatusSelect value={paymentStatus} onChange={setPaymentStatus} />
        </div>
      </section>
      ...
      <CustomerPicker open={customerPickerOpen} onClose={() => setCustomerPickerOpen(false)} onSelect={setCustomer} />
      <ProductPicker open={productPickerOpen} onClose={() => setProductPickerOpen(false)} onSelect={handleSelectProduct} />
    </div>
  );
}
 ...
 * waits for it to load before ever mounting the form, so NewBillForm's
 * state can be initialized synchronously from real data instead of via an
 * effect-based sync.
 */
export function NewBillPage() {
  const { id: routeId } = useParams();
  const { data, isLoading, isError, error, refetch } = useInvoiceQuery(routeId);
    ...
    return (
      <div className="px-4 pt-6">
        <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          <p>Couldn't load this draft — {error.message}</p>
  ...
  return <NewBillForm key={routeId || 'new'} initialInvoice={data?.invoice} />;
}
